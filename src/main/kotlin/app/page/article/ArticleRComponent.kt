...
class ArticleRComponent : RComponent<ArticleProps, ArticleState>() {

    override fun ArticleState.init() {
        MainScope().launch {
            val uuid: String = props.match.params["uuid"] ?: return@launch
            val articleDto: ArticleDto? = ArticleService().fetchByUuid(uuid)

            setState {
                this.articleDto = articleDto
                this.exists = articleDto != null
            }
        }
    }

    override fun RBuilder.render() {
        if (state.exists == null || state.exists == true) {
            DefaultHeader { }
